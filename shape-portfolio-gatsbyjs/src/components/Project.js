...
export const ProjectMetadata = ({startDate, endDate, client}) => {
  ...
  return (
    <div className="project__metadata">
      <p className="project__metadata">{startYear}{includeEndYear ? `&endash; ${endYear}` : ''}</p>
      {client ? <p><a href={client.url} target="_blank" rel="noopener noreferrer">{client.name}</a></p> : ''}
    </div>
  )
}
...
export const Project = ({project}) => (
  <article className="project">
    <header>
      {project.coverImage ? (
        <img className="project__cover-image" alt={project.coverImage.description} src={getImageUrl(project.coverImage.path, {h: 600, w: 1200, fit: 'crop'})} />
      ) : ''}
      <h1>{project.name}</h1>
      <ProjectMetadata
